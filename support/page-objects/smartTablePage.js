...
export class SmartTable {
    ...
    addNewRecordWithFirstAndLastName(firstName, lastName) {
        ...
        cy.get('tbody tr').first().find('td').then(tableBody => {
            cy.wrap(tableBody).eq(2).should('contain', firstName)
            cy.wrap(tableBody).eq(3).should('contain', lastName)
        })
...
export const toSmartTable = new SmartTable()
